...
function SWEP:Think()
	...
	elseif self.guided_nextThink < curtime then
		...
		for k, v in pairs( self.FoundVehicles ) do
			if IsValid( v ) then
				...
				if Ang < 30 and self:CanSee( v ) then
					...
					local stuff = WorldToLocal( v:GetPos(), Angle( 0, 0, 0 ), startpos, Owner:EyeAngles() + Angle(90,0,0) )
					stuff.z = 0
...
function SWEP:PrimaryAttack()
	...
	local ent = ents.Create( "lunasflightschool_missile_alt" )
	ent:SetPos( startpos )
